...
export class Stage1 extends GameStage {
	...
	register() {
		...
		this.iLoader.addImage(ATLAS["192x192"], "./assets/tinyswords192x192.png");
		...
		this.iLoader.addImage(ATLAS["192Units"], "./assets/Warriors192x192.png");
    ...
    init() {
    	...
    	// x, y, width, height, imageKey
       	const water = this.draw.tiledLayer("water", "level1_map", true),
		...
		this.#treesLayer = this.draw.tiledLayer("trees", "level1_map");
		this.#treesLayer.sortIndex = 4;
		...
		this.#enemyBuildings.push(goblinHouse1);
		this.#enemyBuildings.push(goblinHouse2);
		this.#enemyBuildings.push(goblinTower);
		/*
	...
	#render = () => {
		...
		if (eUnitsLen === 0 && eBuildingsLen === 0 && this.#isWin === false) {
	...
	#createGoldBag = (e) => {
		...
		const goldBag = this.draw.gold(peasant.x, peasant.y);
		goldBag.sortIndex = 1;
	...
	#createWoodBunch = (e) => {
		...
		const woodBunch = this.draw.wood(peasant.x, peasant.y);
